/// <summary>
/// Codeunit Budgetary Control (ID 52178700).
/// </summary>
codeunit 52178537 "Budgetary Control"
{
            ...
            IF PurchLine.FINDFIRST THEN BEGIN
                REPEAT
                    IF PurchLine.Type <> PurchLine.Type::" " THEN BEGIN
                        ...
                        CommitmentAmount := 0;
                        Commitments.RESET;
                        Commitments.SETCURRENTKEY(Commitments.Budget, Commitments."G/L Account No.", Commitments."Posting Date"
                      );
                        /*  Commitments.SETCURRENTKEY(Commitments.Budget, Commitments."G/L Account No.",Commitments."Posting Date",
                          Commitments."Shortcut Dimension 1 Code", Commitments."Shortcut Dimension 2 Code",
                        ...
                        END ELSE BEGIN
                            ...
                            END;
                        END;
                        /*  Message('Commited amount ', +CommitmentAmount);
                         Message('Line amount ', +PurchLine."Outstanding Amount (LCY)");
                         Message('Actual amount ', +ActualsAmount);
                         Message('Budget amount ', +BudgetAmount); */
    ...
    begin
        ...
        IF BCSetup.Mandatory THEN//budgetary control is mandatory
          BEGIN
            ...
            //get the lines related to the payment header
            PayLine.RESET;
            PayLine.SETRANGE(PayLine."Document No.", ImprestHeader."No.");
            PayLine.SETRANGE(PayLine."Budgetary Control A/C", TRUE);
            IF PayLine.FINDFIRST THEN BEGIN
                REPEAT
                    //check the votebook now
                    ...
                    AND NOT (BCSetup."Allow OverExpenditure") THEN BEGIN
                        ERROR('The Amount Voucher No %1  %2 %3  Exceeds The Budget By %4',
                        PayLine."Document No.", 'Staff Imprest', PayLine."Document No.",
                          FORMAT(ABS(BudgetAmount - (CommitmentAmount + PayLine."Amount LCY"))));
                    END ELSE BEGIN
                        //ADD A CONFIRMATION TO ALLOW USER TO DECIDE WHETHER TO CONTINUE
                        IF ((CommitmentAmount + PayLine."Amount LCY" + ActualsAmount) > BudgetAmount) THEN BEGIN
