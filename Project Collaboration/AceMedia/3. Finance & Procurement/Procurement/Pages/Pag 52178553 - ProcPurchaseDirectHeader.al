page 52178553 "Proc-Purchase Direct Header"
{
    ...
    layout
    {
        area(content)
        {
            ...
            group("Direct Procurement  Information")
            {
                Editable = Editability;
                //ShowCaption = false;
        ...
        area(processing)
        {
            group("Vendors")
            {
                ...
                action("Selected Vendor(s)")
                {
                    ...
                    begin
                        Vends.RESET;
                        Vends.SETRANGE(Vends."Document Type", Vends."Document Type"::"Direct Procurement");
                ...
                action("Send For Approval")
                {
                    ...
                    begin
                        ...
                        if vends.Findset() then begin
                            VendNo := vends.Count;
                            if (VendNo < 1) or (VendNo > 1) then Error('Only one supplier is required, selected are %1', VendNo);
                        end;
    ...
    begin
        ...
        Rec."Procurement methods" := Rec."Procurement methods"::"Direct Procurement";
        Initializevariables();
        if rec."No." = '' then
            Purchasespaybles.reset();
        Purchasespaybles.SetRange("Doc Type", Purchasespaybles."Doc Type"::"Direct Procurement");
