page 52179118 "HRM-Employee-List"
{
    ...
    SourceTableView = SORTING("No.") ORDER(Ascending) where(Status = filter(active), "Payroll Posting Group" = filter('PAYROLL'));
    ...
    actions
    {
        ...
        area(reporting)
        {
            action(View2PagePayslip)
            {
                ...
                begin
                    ...
                    if SalCard.Find('-') then
                        REPORT.Run(report::"Individual Payslip(Horizontal)", true, false, SalCard);
                end;
            }
            ...
            {
                ApplicationArea = All;
                Caption = 'Import to Salary Card';
                Image = Import;
                Promoted = true;
                PromotedIsBig = true;
                RunObject = xmlport "Salary Card Import";
                //Basic Salary"
            }
            action("Basic Salary")
            {
    ...
    begin
        ...
        USetup.SETRANGE(USetup."Can View Payroll", false);
        IF USetup.FIND('-') THEN ERROR('You are not authorised to view Payroll');
