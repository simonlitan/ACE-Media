page 52178641 "Proc-Low Value Card"
{
            ...
            group(General)
            {
                ...
                field("Requisition No."; Rec."Requisition No.")
                {
                    ...
                    begin
                        cashoffice.Get();
                        PurchLine.RESET;
                        PurchLine.SETRANGE("Document No.", Rec."Requisition No.");
                        IF PurchLine.FIND('-') THEN BEGIN
                            PurchLine.CalcSums("Line Amount");
                            MaxAmount := 100000;
                            if PurchLine."Line Amount" > MaxAmount then Error('Low Value can only be done for requisitions below %1', MaxAmount);
                        END;
                        PurchLine.RESET;
                        PurchLine.SETRANGE("Document No.", Rec."Requisition No.");
                        IF PurchLine.FIND('-') THEN BEGIN
                            repeat
                                if (PurchLine.Type <> PurchLine.type::Item) or (PurchLine.Type <> PurchLine.type::"G/L Account") then Error('Low value is only for items and services');

                            until PurchLine.Next() = 0;
                        END;
                        QuoteLine.Reset();
                        QuoteLine.SetRange("Document No.", rec."No.");
                        QuoteLine.SetRange(Type, QuoteLine.Type::Item);
                        QuoteLine.SetRange("No.", QuoteLine."No.");
                        QuoteLine.SetFilter("Order Date", '%1%2', cashoffice."Current Budget Start Date", cashoffice."Current Budget End Date");
                        if QuoteLine.find('-') then begin
                            repeat
                                Itemno := QuoteLine.Count;
                                if Itemno > 0 then Error('The item %1 has already been purchased in the financial year %2', QuoteLine."No.", cashoffice."Current Budget");
    ...
    begin
        ...
        Rec."Procurement methods" := Rec."Procurement methods"::"Low Value Procurement";
        Initializevariables();
        if rec."No." = '' then
            Purchasespaybles.reset();
        Purchasespaybles.SetRange("Doc Type", Purchasespaybles."Doc Type"::"Low Value Procurement");
