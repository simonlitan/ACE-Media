page 52179168 "HRM-Employee Requisition Card"
{
    DeleteAllowed = false;
    InsertAllowed = true;
    PageType = Card;
    ...
    actions
    {
        area(navigation)
        {
            ...
            action("&Send Approval Request")
            {
                ...
                trigger OnAction()
                var
                    Approvalmgt: Codeunit EmpReqIntCodeunit;
                begin
                    if Confirm('Send this Requisition for Approval?', true) = false then exit;

                    TESTFIELDS;
                    if Approvalmgt.IsEmpReqEnabled(Rec) = true then
                        Approvalmgt.OnSendEmpReqforApproval(Rec);
            ...
            action("&Cancel Approval Request")
            {
                ...
                begin
                    ...
                    if Approvalmgt.IsEmpReqEnabled(Rec) = true then
                        Approvalmgt.OnCancelEmpReqforApproval(Rec);
