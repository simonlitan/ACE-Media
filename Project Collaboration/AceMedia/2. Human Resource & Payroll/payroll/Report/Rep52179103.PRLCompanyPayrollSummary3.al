report 52179103 "PRL-Company Payroll Summary 3"
{
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem(prPeriod_Transactions; "PRL-Period Transactions")
        {
            ...
            dataitem("PRL-Employer Deductions"; "PRL-Employer Deductions")
            {
                DataItemLink = "Payroll Period" = field("Payroll Period");
                column(TransactionCode_PRLEmployerDeductions; "Transaction Code")
                {
                }
                column(Amount_PRLEmployerDeductions; Amount)
                {
                }
                trigger OnPreDataItem()
                begin
                    "PRL-Employer Deductions".SetFilter("Payroll Period", '=%1', PeriodFilter);
                end;
            }
            ...
            begin
                ...
                IF ((prPeriod_Transactions."Group Order" = 9) AND (prPeriod_Transactions."Sub Group Order" = 0) AND (prPeriod_Transactions."Transaction Code" = 'NPAY')) THEN
                ...
                IF ((prPeriod_Transactions."Group Order" = 8) AND ((prPeriod_Transactions."Sub Group Order" = 1) OR (prPeriod_Transactions."Sub Group Order" = 0))) THEN
                    IF (((prPeriod_Transactions."Group Order" = 7) AND ((prPeriod_Transactions."Sub Group Order" = 3) OR (prPeriod_Transactions."Sub Group Order" = 1) OR
                        ...
                        IF (trans."Transaction Code" = 'D004') THEN
                            NSSF := trans.Amount;
                CLEAR(TransTypes);
                IF (prPeriod_Transactions."Group Text" IN ['INCOME', 'EARNINGS', 'ALLOWANCE', 'ALLOWANCES', 'BASIC SALARY', 'GROSS PAY', 'NSSF EMPLOYER']) THEN TransTypes := 'PAYMENTS';
                ...
                IF (prPeriod_Transactions."Group Text" IN ['INCOME', 'EARNINGS', 'ALLOWANCE', 'ALLOWANCES', 'BASIC SALARY', 'GROSS PAY', 'NSSF EMPLOYER']) THEN PaymentAmount := prPeriod_Transactions.Amount;
    ...
    var
        ...
        EmployerContr: record "PRL-Employer Deductions";
        SelectedPeriod: Date;
        PeriodName: Text[30];
