page 52178819 "Tender Technical QualifiedList"
{
    CardPageID = "Tender Technical QualifiedCard";
    Editable = true;
    PageType = ListPart;
    SourceTable = "Tender Submission Header";
    // SourceTableView = where("Technical Qualified" = filter(true));
    layout
    {
        area(content)
        {
        ...
        area(processing)
        {
            group("&Process")
            {
                ...
                action("Assign Vendor(s)")
                {
                    ...
                    begin
                        Vends.RESET;
                        Vends.SETRANGE(Vends."Document Type", Vends."Document Type"::"Open Tendering");
