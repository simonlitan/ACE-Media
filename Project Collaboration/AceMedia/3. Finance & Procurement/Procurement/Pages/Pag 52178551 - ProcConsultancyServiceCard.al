page 52178551 "Proc-Consultancy Service Card"
{
        ...
        area(processing)
        {
            group("Vendors")
            {
                action("Select Vendors")
                {
                    ...
                    begin
                        IF rec.Status = rec.Status::Open then
                            ...
                            Error('Can only assign vendors if it is Open');
                    end;
                }
                action("Selected Vendor(s)")
                {
                    ...
                    begin
                        Vends.RESET;
                        Vends.SETRANGE(Vends."Document Type", Vends."Document Type"::"Consultancy Services");
    ...
    begin
        ...
        Rec."Procurement methods" := Rec."Procurement methods"::"Consultancy Services";
        Initializevariables();
        if rec."No." = '' then
            Purchasespaybles.reset();
        Purchasespaybles.SetRange("Doc Type", Purchasespaybles."Doc Type"::"Consultancy Services");
