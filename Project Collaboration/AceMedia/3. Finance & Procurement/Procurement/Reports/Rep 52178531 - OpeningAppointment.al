report 52178577 "Opening Appointment"
{
    Caption = 'Opening Appointment';
    DefaultLayout = RDLC;
    RDLCLayout = './Procurement/Reports/SSR/Opening Appointment.rdl';
    dataset
    {
            ...
            area(content)
            {
                group(GroupName)
                {
                    field(MemberNo; MemberNo)
                    {
                        Caption = 'Member No';
                        ApplicationArea = all;
                        TableRelation = "Proc-Committee Members"."Member No" where(Committee = filter(Opening));
