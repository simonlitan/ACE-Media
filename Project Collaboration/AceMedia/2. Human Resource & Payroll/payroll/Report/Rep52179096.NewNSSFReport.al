/// <summary>
/// Report New NSSF Report (ID 50015).
/// </summary>
report 52179096 "New NSSF Report"
{
    Caption = 'New NSSF Report';

    DefaultLayout = RDLC;
    RDLCLayout = './payroll/Report/SSR/New NSSF Report.rdl';
    ...
    dataset
    {
        dataitem("HRM-Employee C"; "HRM-Employee C")
        {
            ...
            column(NSSFAmount1; NSSFAmount1)
            {

            }
            column(NSSFAmount2; NSSFAmount2)
            {

            }
            column(Gtoto2; Gtoto2)
            {
            ...
            begin
                ...
                if PRLPeriodTransactions.Find('-') then begin
                    NSSFAmount1 := PRLPeriodTransactions.Amount;
                end;

                //Shares
                PRLPeriodTransactions.Reset;
                PRLPeriodTransactions.SetRange(PRLPeriodTransactions."Employee Code", "HRM-Employee C"."No.");
                PRLPeriodTransactions.SetRange(PRLPeriodTransactions."Transaction Code", 'NSSF(II)');
                PRLPeriodTransactions.SetRange(PRLPeriodTransactions."Group Text", 'STATUTORIES');
                PRLPeriodTransactions.SetRange(PRLPeriodTransactions."Payroll Period", datefilter);
                if PRLPeriodTransactions.Find('-') then begin
                    NSSFAmount2 := PRLPeriodTransactions.Amount;
                end;
                ...
                */
                ...
                Gtoto := NSSFAmount1 + NSSFAmount2;//+PRLEmployerDeductions.Amount;
                if Gtoto = 0 then CurrReport.Skip else seq := seq + 1;
                Gtoto2 := Gtoto * 2;
                if Gtoto2 = 0 then CurrReport.Skip else seq := seq + 1;
