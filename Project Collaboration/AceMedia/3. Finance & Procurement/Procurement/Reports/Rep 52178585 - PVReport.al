report 52178585 "PV Report"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/SSR/Payment Voucher layout.rdl';

    dataset
    {
        dataitem(PaymentHeader; "FIN-Payments Header")
        {
            CalcFields = "Total Net Amount", Amount;
            DataItemTableView = SORTING("No.");
            RequestFilterFields = "No.";
            column(UserAppId; userSet."User ID")
            {
            ...
            column(TotalNetAmount_PaymentHeader; "Total Net Amount")
            {
            }
            column(Amount_PaymentHeader; Amount)
            {
            }
            ...
            column(PayingBankAccount_PaymentHeader; "Paying Bank Account")
            {
            }
            ...
            column(BankAccountNumber_PaymentHeader; "Bank Account Number")
            {
            }
            column(MainBankName_PaymentHeader; "Main Bank Name")
            {
            }
            column(BranchName_PaymentHeader; "Branch Name")
            {
            }
            ...
            column(AdvanceAmount_PaymentHeader; "Advance Amount")
            {
            }
            column(AdvancedAmount_PaymentHeader; "Advanced Amount")
            {
            }
            ...
            column(utitle; HRMEMPLOYEEC."Job Title")
            {

            }
            column(PINNO_PaymentHeader; "PIN  NO")
            {
            }
            ...
            dataitem("<Payment Line>"; "FIN-Payment Line")
            {
                ...
                column(Budgeted_Allocation; "Budgeted Allocation")
                {

                }
                column(Budgeted_Allocation1; "Budgeted Allocation1")
                {

                }
                column(TotalExpenseCommit; "Budgeted Allocation" - ("Budget Balance" + Amount2))
                {

                }
                // column(TotalExpenseCommit1; "Budgeted Allocation1" - "Budget Balance")
                // {

                // }

                column(Budget_Balance; "Budget Balance" + Amount2) 
                {

                } 
                column(Budget_Balance1; "Budget Balance" - Amount3)
                { 
                ...
                column(BudgetedAcc; "Budgeted Acc")
                {
                ...
                dataitem(FINBudgetEntries; "FIN-Budget Entries")
                {

                    DataItemLink = "Budget Name" = FIELD("Budget Name");

                    column(BudgetAllocation22; BudgetAllocation22)
                    {
                    ...
                    begin
                        ...
                        FINBudgetEntries.setrange("G/L Account No.", paymentline."Account No.");
                        FINBudgetEntries.setrange("Budget Name", paymentline."budget name");
                        FINBudgetEntries.SETFILTER("Transaction Type", '%1|%2|%3', FINBudgetEntries."Transaction Type"::Allocation, FINBudgetEntries."Transaction Type"::Commitment, FINBudgetEntries."Transaction Type"::Expense);
                        if FINBudgetEntries.find('-') then begin
                            if FINBudgetEntries.CALCSUMS(Amount) then begin
                                BudgetAllocation22 := FINBudgetEntries."Transaction Type"::Allocation
                            end;
                ...
                dataitem(vendLedger; "Vendor Ledger Entry")
                {
                    ...
                    column(DocumentNo_vendLedger; "Document No.")
                    {
                    }
                    column(ExternalDocumentNo_vendLedger; "External Document No.")
                    {
                    }
                    column(ClosedbyAmount_vendLedger; Amount)
                    {
                    }
                }
            ...
            dataitem(ApprovalEntry; "Approval Entry")
            {
                ...
                dataitem(UserSetUp; "User Setup")
                {
                    ...
                    column(UserSignId; UserSetUp."User ID")
                    {
                    ...
                    begin
                        ...
                        // UserSetUp.SetRange("Approval Role", 'Administrator');
            ...
            begin
                ...
                // BudgetBalAcct.Get(paymentline."Account No.");
                // BudgetBalAcct.CalcFields(Allocation)
                // if BudgetBalAcct.find('-') then  

                // BudgetBalAccNo := BudgetBalAcct."G/L Account No."; 
                ...
                // WordTotal := PaymentHeader."Total Net Amount";
                WordTotal := PaymentHeader.Amount;

                //End;
                //PaymentHeader.CALCFIELDS("Total Payment Amount", "Total Witholding Tax Amount", "Total Net Amount");

                InitTextVariable;
                FormatNoText(NumberText, WordTotal, CurrencyCodeText);
                ...
                if ApprovalEntry4.FindFirst() then begin
                    ...
                    //get approval username and signature
                            UserSetup.Reset();
                UserSetup.SetRange("User ID", PaymentHeader.Cashier);
                if UserSetup.Find('-') then
            ...
            begin
                ...
                //Budget
                // BudgetSummary.reset();
                // BudgetSummary.SetRange("G/L Account No.", "account no.");
                // if BudgetSummary.Find('-') then begin
                //     BudgetSummary.CalcFields(Allocation);
                //     BudgetAllocation1 := BudgetSummary.allocation;
    ...
    var
        PaymentHeader1: Record "FIN-Payments Header";
        ...
        BudgetAllocation22: Decimal;
        TTamount2: Decimal;
        TTNetamount2: Decimal;
        TTwithAmount2: Decimal;
        TTwithAmountVat2: Decimal;
        WordTotal: Decimal;
    ...
    local procedure AddToNoText(var NoText: array[2] of Text[200]; var NoTextIndex: Integer; var PrintExponent: Boolean; AddText: Text[200])
    begin
        PrintExponent := true;
