report 52179115 "Individual Payslips V.1.1.3"
{
    DefaultLayout = RDLC;
    RDLCLayout = './payroll/Report/SSR/Nepad Payslips V.1.1.3.rdl';
    ...
    dataset
    {
        dataitem("PRL-Period Transactions"; "PRL-Period Transactions")
        {
            DataItemTableView = SORTING("Employee Code", "Period Month", "Period Year", "Group Order", "Sub Group Order", Membership, "Reference No")
              ORDER(Ascending);
            RequestFilterFields = "Employee Code", "Payroll Period", "Payroll Code";
            column(FullNames; emp1."First Name" + ' ' + emp1."Middle Name" + ' ' + emp1."Last Name")
            {
            }
            column(ContractStartDate;ContractStartDate)
            {

            }
            column(AnnualIncreament;AnnualIncreament)
            {
            ...
            column(IDnNHIF; strNhifNo)
            {
            }
            column(JobGroup; strJobG)
            {
            }
           ...
           // }
            ...
            begin
                ...
                IF objEmp.FIND('-') THEN BEGIN
                    ...
                    strJobG := objEmp."Salary Grade";
                    IF objEmp."ID Number" <> '' THEN
                        strID := objEmp."ID Number";
                        if objEmp."Job Title" <> '' then
                        Designation := objEmp."Job Title";
                    IF objEmp."PAYE Number" <> '' THEN
                        strPin := objEmp."PAYE Number";
                        // if objEmp.dat <> 0D then
                        // ContractStartDate := objEmp."Contract Start Date";
                        // ContractEnd := objEmp."Contract End Date";
                        if objEmp."Date of Current Appointment" <> 0D then

                          ContractStartDate := objEmp."Date of Current Appointment";
                        ContractEnd := objEmp."Contract End Date";
                        if objEmp."Annual Increament Date" <> 0D then
                        AnnualIncreament := objEmp."Annual Increament Date";
                    ...
                    IF objEmp."Contract End Date" = 0D THEN
                        dtOfLeaving := DMY2DATE(31, 12, 9999)
                    ELSE
                        // dtOfLeaving := objEmp."Date Of Leaving";
                        dtOfLeaving := objEmp."Contract End Date";
                    ...
                    IF ((objEmp."Date Of Birth" <> 0D) AND (objPeriod."Date Opened" <> 0D)) THEN
                        strAge := HrDate.DetermineAge_Years(objEmp."Date Of Birth", objPeriod."Date Opened");
                          //if objEmp."No." <> UserId then Error('Warning!! You can only view your Payslip');
                ...
                END;
                // UserSetup.Reset();
                // UserSetup.SetRange("Employee No.", objEmp."No.");
                // if UserSetup.Find('-') then
                // begin
                //       if objEmp."No." <> UserSetup."Employee No." then Error('Warning!! You can only view your Payslip');
                // end;
