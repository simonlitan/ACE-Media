page 52178673 "FIN-Staff Claims"
{
    DeleteAllowed = false;
    PageType = Card;
    ...
    PromotedActionCategories = 'New,Process,Report,Approvals,Posting,Attachments';
    ...
    actions
    {
        area(processing)
        {
            action("File Attachment")
            {
                ...
                begin
                    DMS.Reset;
                    DMS.SetRange("Document Type", DMS."Document Type"::"Staff Claims");
                    if DMS.Find('-') then begin
                        Hyperlink(DMS."url path" + Rec."No.");
                    end else
                        Message('No Link ' + format(DMS."Document Type"::"Staff Claims"));
                end; */
            ...
            action(sendApproval)
            {
                ...
                begin
                    if rec.Purpose = '' then error('Claim Description cannot be empty');
                    DocAtt.Reset();
                    DocAtt.SetRange("No.", rec."No.");
                    if DocAtt.Count < 1 then Error('No attachments done');
                    if ApprovalMgnt.IsClaimsEnabled(Rec) = true then
                        ApprovalMgnt.OnSendClaimforApproval(Rec);
                    TravReqHeader.CommitBudget();
            ...
            action(cancellsApproval)
            {
                ...
                begin
                    ...
                    //if ApprovalMgnt.IsClaimsEnabled(Rec) then
                    ApprovalMgnt.OnCancelClaimForApproval(Rec);
                    TravReqHeader.ReverseBudget();
                end;
            }
            ...
            action("Print/Preview")
            {
                ...
                begin
                    ...
                    REPORT.RUN(Report::"Claim Requisition", TRUE, TRUE, Rec);
    ...
    begin
        ...
        IF PayLine.FIND('-') THEN BEGIN
            REPEAT
                ...
                GenJnlLine."Document No." := Rec."No.";
                GenJnlLine."External Document No." := Rec."Account No.";
